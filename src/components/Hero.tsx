...
export default function Hero (): ReactElement {
  ...
  return (
    <motion.section ref={ref} className="h-[600px] relative overflow-hidden ">
      <div className={`flex justify-around  space-x-20a   mt-44 z-10 items-center ${ screen === "mobile" ? "flex-col-reverse justify-center" : "" }`}>
        <div className={` ${ screen === "mobile" ? "w-96 text-center" : "w-[40%]" } space-y-5 flex flex-col justify-center items-center`}>
          <div>
            <motion.h3 whileInView={{x:0,opacity:1}} transition={{duration:.4}} initial={{ x: screen !== "mobile" ? -200 : 0 ,opacity:0}} className="text-primary font-bold">Moz Agency</motion.h3>
          ...
          <motion.div whileInView={{opacity:1}} transition={{duration:.4,delay:.4}} initial={{ opacity: screen !== "mobile" ? 0 : 1 }} className={`flex justify-around w-full -ml-14 ${ screen === "mobile" ? "ml-0" : "" } items-center`}>
            <a href="#">
